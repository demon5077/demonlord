  ...
  return (
    <div className="pb-12">
      ...
      <div className="relative h-[40dvh] w-full overflow-hidden md:h-[55dvh]">
        ...
        {trailer && (
          <div className="absolute inset-0 flex cursor-pointer items-center justify-center opacity-0 transition-opacity hover:opacity-100">
            <a
      ...
      <div className="relative z-10 mx-auto -mt-36 max-w-5xl space-y-10 px-4 md:px-6">
        <div className="flex flex-col gap-6 md:flex-row">
          ...
          <article className="flex flex-col gap-4 md:pt-32">
            ...
            >
              {data.release_date && (
                <span>{format(new Date(data.release_date), 'MMMM d, yyyy')}</span>
              )}
              {runtime && (
                <>
                  <span>·</span>
              ...
              {data.status && (
                <>
                  ...
                  <span
                    style={{
                      color:
                        data.status === 'Released' ? 'var(--neon-green)' : 'var(--neon-yellow)',
                    }}
                  >
                    {data.status}
                  </span>
              ...
              {data.vote_average > 0 && (
                ...
                >
                  ...
                  <span className="text-xs text-[hsl(var(--muted-foreground))]">
                    / 10 ({data.vote_count?.toLocaleString()})
                  </span>
                </div>
              )}
            ...
            {director && (
              ...
              >
                Directed by{' '}
                <span className="font-semibold text-[hsl(var(--foreground))]">{director.name}</span>
              </p>
            )}
            ...
            <div className="grid grid-cols-4 gap-3 sm:grid-cols-6 md:grid-cols-8">
              {topCast.map((c: any) => (
                <div key={c.id} className="flex flex-col items-center gap-1.5 text-center">
                  ...
                  <p className="line-clamp-1 w-full text-[0.65rem] leading-tight font-semibold">
                    {c.name}
                  </p>
                  <p className="line-clamp-1 w-full text-[0.58rem] leading-tight text-[hsl(var(--muted-foreground))]">
                    {c.character}
                  </p>
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
              {similar.map((s: any) => (
                <Link key={s.id} href={`/movie/${s.id}`} className="group">
                  <div className="relative aspect-[2/3] overflow-hidden rounded-xl border border-[hsl(var(--border))] transition-all duration-200 group-hover:border-[var(--neon-pink)]">
                    {s.poster_path ? (
                      <Image
                        fill
