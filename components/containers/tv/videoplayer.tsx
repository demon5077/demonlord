  ...
  return (
    <div className="space-y-4 py-4">
      ...
      <div className="flex flex-wrap items-center gap-2 px-2">
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={10}
              className={`${seasonOpen ? 'rotate-180' : ''} transition-transform ${textClass}`}
            />
          </button>
          {seasonOpen && (
            <div
              className={`absolute top-full left-0 z-50 mt-1 max-h-48 w-36 overflow-y-auto border shadow-xl ${
        ...
        {/* Prev/Next Episode */}
        <button
          onClick={() => prevEp && setEpisode(prevEp.episode_number.toString())}
          disabled={!prevEp}
          className={`border p-2 transition-colors disabled:opacity-30 ${borderClass} hover:border-[#00D4FF]/40`}
        >
          <ChevronLeft size={12} className={textClass} />
        </button>

        <div
          className={`min-w-[72px] border px-3 py-2 text-center text-[0.65rem] tracking-widest ${borderClass}`}
          style={monoStyle}
        >
        ...
        <button
          onClick={() => nextEp && setEpisode(nextEp.episode_number.toString())}
          disabled={!nextEp}
          className={`border p-2 transition-colors disabled:opacity-30 ${borderClass} hover:border-[#00D4FF]/40`}
        >
        ...
        <div className="relative">
          ...
          >
            ...
            {currentServer.tag && (
              <span className="text-[0.55rem] text-[#F9F002]">[{currentServer.tag}]</span>
            )}
            <ChevronDown
              size={10}
              className={`${serverOpen ? 'rotate-180' : ''} transition-transform ${textClass}`}
            />
          </button>
          {serverOpen && (
            <div
        ...
        {/* Download */}
        <Link
          href={`https://dl.vidsrc.vip/tv/${id}/${season}/${episode}`}
          target="_blank"
      ...
      <div className={`player-container ${isLight ? 'border border-gray-200' : ''}`}>
        {isLoading ? (
          <div
            className={`flex h-[520px] items-center justify-center ${isLight ? 'bg-gray-50' : 'bg-[#080B14]'}`}
          >
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-[#FF006F] border-t-transparent" />
          </div>
        ) : adBlock ? (
      ...
      {episodes.length > 0 && (
        <div className="space-y-2 px-2">
          <span
            className={`text-[0.6rem] tracking-[0.25em] uppercase ${isLight ? 'text-gray-400' : 'text-white/30'}`}
            style={monoStyle}
          >
          ...
          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {episodes.map((ep) => (
              ...
              >
                ...
                <span
                  className={`mt-0.5 w-full truncate text-xs ${isLight ? 'text-gray-600' : 'text-white/60'}`}
