...
export default function FeaturedTV({ featureType }: { featureType: FeatureType }) {
  ...
  if (error) return (
    <div className="flex flex-col items-center gap-3 py-16 border border-[hsl(0_85%_60%/0.3)] bg-[hsl(0_85%_60%/0.05)]">
      ...
      <span className="text-[0.65rem] tracking-widest text-[hsl(0_85%_60%)]" style={{ fontFamily: 'Share Tech Mono, monospace' }}>{error}</span>
    </div>
  );
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
      {items.map(item => (
        <Link key={item.id} href={`/tv/${item.id}`} className="group">
          <div className="relative aspect-[2/3] overflow-hidden border border-[hsl(var(--border))] group-hover:border-[hsl(185_100%_48%/0.5)] transition-all duration-300">
            {item.poster_path ? (
              <Image fill src={`https://image.tmdb.org/t/p/w342${item.poster_path}`}
                alt={item.name} sizes="200px"
            ...
            {item.vote_average > 0 && (
              <div className="absolute top-1.5 right-1.5 flex items-center gap-1 bg-black/80 px-1.5 py-0.5 border border-[hsl(185_100%_48%/0.3)]">
                <Star size={8} className="text-[hsl(185_100%_48%)] fill-current" />
                <span className="text-[0.6rem] text-[hsl(185_100%_48%)]" style={{ fontFamily: 'Share Tech Mono, monospace' }}>
            ...
            <div className="absolute bottom-0 inset-x-0 p-2 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
              <p className="text-xs font-semibold text-white truncate" style={{ fontFamily: 'Rajdhani, sans-serif' }}>{item.name}</p>
            </div>
          </div>
        </Link>
      ))}
