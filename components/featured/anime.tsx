  ...
  if (loading) return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={i} className="aspect-[2/3] bg-white/5 animate-pulse border border-white/5" />
      ))}
    </div>
  );

  if (error) return (
    <div className="flex flex-col items-center gap-3 py-12 border border-[#FF006F]/20 bg-[#FF006F]/5">
      <AlertCircle size={20} className="text-[#FF006F]" />
      <span className="text-[0.6rem] tracking-widest text-[#FF006F]" style={{ fontFamily: 'Share Tech Mono, monospace' }}>{error}</span>
    </div>
  );
      ...
      {items.map(item => {
        ...
        return (
          <Link key={item.id} href={`/anime/${item.id}`} className="group">
            <div className="relative aspect-[2/3] overflow-hidden border border-white/8 group-hover:border-[#BD00FF]/50 transition-all duration-300">
              {img ? (
                <Image fill src={img} alt={title} sizes="200px"
              ...
              {score > 0 && (
                <div className="absolute top-1.5 right-1.5 flex items-center gap-1 bg-black/80 px-1.5 py-0.5 border border-[#BD00FF]/40">
                  <Star size={8} className="text-[#BD00FF] fill-current" />
                  <span className="text-[0.6rem] text-[#BD00FF]" style={{ fontFamily: 'Share Tech Mono, monospace' }}>
                    {score.toFixed(1)}
                  </span>
                </div>
              )}
              <div className="absolute bottom-0 inset-x-0 p-2 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                <p className="text-xs font-semibold text-white truncate" style={{ fontFamily: 'Rajdhani, sans-serif' }}>{title}</p>
              </div>
            </div>
          </Link>
        );
